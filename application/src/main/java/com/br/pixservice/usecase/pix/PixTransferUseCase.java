...
public class PixTransferUseCase {
    ...
    @Value(value = "${app.rabbitmq.exchange.pix}")
    private String pixExchange;
    @Value(value = "${app.rabbitmq.routing}")
    ...
    public PixTransfer execute(String idempotencyKey, String fromWalletId, String targetPixKey, BigDecimal amount) {
        ...
        try {
            ...
            PixKey receiverKey = pixKeyRepository.findByKey(targetPixKey)
                    .orElseThrow(() -> new IllegalArgumentException(
            ...
            publisher.publish(pixExchange, routingKey, objectMapper.writeValueAsString(transfer));
